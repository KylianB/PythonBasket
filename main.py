...
class Ball(pygame.sprite.Sprite):
    ...
    def preview(self):
        global alphaRad, pts_list
        pts_list = [[self.position[0], self.position[1]]]
        vitesse_prev = [math.cos(alphaRad) * force, -math.sin(alphaRad) * force]
        for i in range(20):
            pos = [0, 0]
            self.position[0] = pos[0]
            self.position[1] = pos[1]

            pos[0] += vitesse_prev[0]
            pos[1] += vitesse_prev[1]

            vitesse_prev[1] += 4
            vitesse_prev[0] *= 0.99
            vitesse_prev[1] *= 0.99

            pts_list += [[pos[0], pos[1]]]

    def evolution(self):
        global vitesse, start
